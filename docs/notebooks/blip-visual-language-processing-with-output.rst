...
`BLIP <http/github.csalesforBLIP>`__ is a language-image
...
`NNCF <http/github.copenvinotoolknncf>`__.
...
paper <http/arxiv.oa2201.12086>`__, `Salesforce
blog <http/blog.salesforceairesearch.cblip-bootstrapping-language-image-pretraini>`__,
`GitHub repo <http/github.csalesforBLIP>`__ and `Hugging Face
model
documentation <https://huggingface.co/docs/transformers/model_doc/blip>`__.

In this tutorial, you will use the
`blip-vqa-base <http/huggingface.Salesforblip-vqa-base>`__
model available for download from `Hugging
Face <http/huggingface.>`__. The same actions are also applicable
...
.. code:: ipython3
    ...
    %pip install -q --extra-index-url https://download.pytorch.org/whl/cpu torch torchvision "transformers>=4.26.0" gradio "openvino>=2023.3.0" "datasets>==2.14.6" "nncf>=2.8.1"
...
`blog <http/huggingface.blhow-to-generate>`__. The entry point
...
`documentation <http/huggingface.dotransformev4.26main_classtext_generation#transformers.GenerationMixin.generate>`__.
...
`article <http/scale.cblpytorch-improvements#Text%20Translation>`__.
...
The `VQAv2 <http/visualqa.o>`__ is a dataset containing
...
Open the `blip-optimize <blip-optimize-with-output.html>`__ notebook to quantize
